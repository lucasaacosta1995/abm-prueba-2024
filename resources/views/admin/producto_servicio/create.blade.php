            ...
            <form action="{{ route('productos-servicios.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="id_rubro" name="id_rubro" required>
                        ...
                        @foreach($rubros as $rubro)
                            <option @if($rubro->id == old('id_rubro')) selected
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="tipo" name="tipo" required>
                        ...
                        @foreach($tipos as $tipoId => $tipo)
                            <option @if($tipoId == old('tipo')) selected @endif value="{{$tipoId}}">{{$tipo}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-control" id="id_unidad_medida" name="id_unidad_medida" required>
                        ...
                        @foreach($unidadesMedidas as $unidadMedida)
                            <option @if($unidadMedida->id == old('id_unidad_medida')) selected
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="id_condicion_iva" name="id_condicion_iva" required>
                        ...
                        @foreach($condicionesIva as $condicionIva)
                            <option @if($condicionIva->id == old('id_condicion_iva')) selected
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="precio_bruto_unitario"
                           value="{{ old('precio_bruto_unitario') }}" name="precio_bruto_unitario" required>
                </div>

                <button type="submit" class="btn btn-primary">Crear Producto o Servicio</button>
            </form>
        </div>
    </div>
@endsection
